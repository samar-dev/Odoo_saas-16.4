...
QUnit.module("Views > GanttView", (hooks) => {
    ...
    QUnit.test("Connector buttons: reschedule task backward date.", async (assert) => {
        ...
        assert.verifySteps([
            `["web_gantt_reschedule",["backward",1,2,"depend_on_ids",null,"planned_date_begin","planned_date_end"]]`,
        ]);
    });
    ...
    QUnit.test("Connector buttons: reschedule task forward date.", async (assert) => {
        ...
        assert.verifySteps([
            `["web_gantt_reschedule",["forward",1,2,"depend_on_ids",null,"planned_date_begin","planned_date_end"]]`,
        ]);
    });
