...
export class AccountReportController {
    ...
    useSessionOptions(type) {
        if (!["read", "write"].includes(type))
            throw new Error(`Invalid argument value in useSessionOptions(): ${ type }`);

        // By default, we want to use the session options
        return !["both", type].includes(this.action.params?.ignore_session)
    }

    hasSessionOptions() {
        return this.useSessionOptions("read") && Boolean(browser.sessionStorage.getItem(this.sessionOptionsID()));
    }

    saveSessionOptions(options) {
        if (this.useSessionOptions("write"))
