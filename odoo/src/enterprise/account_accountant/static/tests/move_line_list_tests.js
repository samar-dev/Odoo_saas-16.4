...
QUnit.module("Views", {}, function () {
    ...
    QUnit.test("Fetch and preview of attachments on big devices", async (assert) => {
        ...
        await contains(
            `.o_attachment_preview iframe[data-src='/web/static/lib/pdfjs/web/viewer.html?file=${encodeURIComponent(getOrigin()+'/web/content/1')}#pagemode=none']`
        );
        assert.verifySteps([], "no extra rpc should be done");
        ...
        await contains(
            `.o_attachment_preview iframe[data-src='/web/static/lib/pdfjs/web/viewer.html?file=${encodeURIComponent(getOrigin()+'/web/content/2')}#pagemode=none']`
        );
        assert.verifySteps([], "no extra rpc should be done");
        await click(":nth-child(1 of .o_data_row) :nth-child(2 of .o_data_cell)");
        await contains(".o_attachment_preview iframe", { count: 0 });
