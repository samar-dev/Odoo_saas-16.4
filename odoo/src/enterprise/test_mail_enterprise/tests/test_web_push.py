...
class TestWebPushNotification(SMSCommon):
    ...
    def _trigger_cron_job(self):
        self.env.ref('mail_enterprise.ir_cron_web_push_notification').method_direct_trigger()
    ...
    @patch.object(odoo.addons.mail_enterprise.models.mail_thread, 'push_to_end_point')
    ...
    @patch.object(odoo.addons.mail_enterprise.models.web_push, 'push_to_end_point')
    ...
    @patch.object(odoo.addons.mail_enterprise.models.mail_thread.Session, 'post',
    ...
    def test_push_notifications_device_unreachable(self, post):
        with mute_logger('odoo.addons.mail_enterprise.web_push'):
            self.record_simple.with_user(self.user_email).message_notify(
                partner_ids=self.user_inbox.partner_id.ids,
                body="Test message send via Web Push",
                subject="Test Activity",
