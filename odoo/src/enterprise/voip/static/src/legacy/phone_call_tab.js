...
export const PhoneCallTab = Widget.extend({
    ...
    async callFirstNumber() {
        ...
        if (number) {
            ...
            this.voip.legacyDialingPanelWidget.makeCall(number);
        }
    },
    async callFromDetails(number) {
        this._currentPhoneCallId = this._selectedPhoneCallId;
        const phoneCall = await this._getPhoneCall(this._selectedPhoneCallId);
        this.voip.legacyDialingPanelWidget.makeCall(number, phoneCall);
    },
    /**
    ...
    async callFromTab() {
        if (this._phoneCalls.length > 0) {
            ...
            this.voip.legacyDialingPanelWidget.switchKeypad();
        }
    },
    /**
    ...
    async initPhoneCall() {
        ...
        this.voip.legacyDialingPanelWidget.showHangupButton();
    },
    /**
    ...
    async onIncomingCallAccepted({ number, partnerId }) {
        const phoneCallData = await this.env.services.orm.call(
            "voip.phonecall",
            "create_from_incoming_call_accepted",
            [[this._currentPhoneCallId], number, partnerId]
        );
        const phoneCallId = await this._displayInQueue(phoneCallData);
        this._currentPhoneCallId = phoneCallId;
        await this._selectPhoneCall(phoneCallId);
    ...
    async removePhoneCall(phoneCallId) {
        const resId = await this.env.services.orm.call("voip.phonecall", "remove_from_queue", [
            [phoneCallId],
        ]);
        await this.refreshPhonecallsStatus();
        this.voip.bus.trigger("on-call-activity-removed", resId);
    },
    /**
    ...
    async _closePhoneDetails() {
        ...
        this.voip.legacyDialingPanelWidget.showHeader();
    ...
    _getCurrentPhoneCall() {
        return this._phoneCalls
            .slice()
            .reverse()
            .find((phoneCall) => phoneCall.id === this._currentPhoneCallId);
    },
    /**
    ...
    async _getPhoneCall(phoneCallId) {
        const phoneCall = this._phoneCalls
            .slice()
            .reverse()
            .find((phoneCall) => phoneCall.id === phoneCallId);
        if (phoneCall) {
            return phoneCall;
        }
        const phoneCallData = await this.env.services.orm.call(
    ...
    _getSelectedPhoneCall() {
        return this._phoneCalls
            .slice()
            .reverse()
            .find((phoneCall) => phoneCall.id === this._selectedPhoneCallId);
    },
    /**
    ...
    async _selectPhoneCall(phoneCallId) {
        ...
        this.voip.legacyDialingPanelWidget.hideHeader();
    },
