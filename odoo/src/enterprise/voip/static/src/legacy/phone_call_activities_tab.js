...
export const PhoneCallActivitiesTab = PhoneCallTab.extend({
    /**
     * @override
     */
    init() {
    ...
    async initPhoneCall() {
        ...
        await this.env.services.orm.call("voip.phonecall", "init_call", [
    ...
    async searchPhoneCall(search) {
        ...
        for (const phoneCall of this._phoneCalls) {
            const flagPartner = phoneCall.partnerName && phoneCall.partnerName.search(expr) > -1;
            let flagName = false;
            if (phoneCall.name) {
