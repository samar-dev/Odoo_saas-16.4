...
class WhatsAppMessage(models.Model):
    ...
    def _compute_mobile_number_formatted(self):
        def country_from_message(message):
            recipient_partner = message.mail_message_id.partner_ids[0] if message.mail_message_id.partner_ids else self.env['res.partner']
            return recipient_partner.country_id if recipient_partner.country_id else self.env.company.country_id
        for country, messages in groupby(self, country_from_message):
            messages = self.env['whatsapp.message'].concat(*messages)
            sanitized_numbers = phone_validation.phone_sanitize_numbers(messages.mapped('mobile_number'), country.code, country.phone_code)
            for message in messages:
                sanitized_number = sanitized_numbers.get(message.mobile_number, dict()).get('sanitized', '')
