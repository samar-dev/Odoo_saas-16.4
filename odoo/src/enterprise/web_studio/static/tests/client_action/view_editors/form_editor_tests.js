...
QUnit.module("View Editors", (hooks) => {
    ...
    QUnit.test('Form readonly attribute should set force_save to True', async function (assert) {
        ...
        await createViewEditor({
            ...
            mockRPC: function(route, args) {
                if (route === "/web_studio/edit_view") {
                    if (step == 0) {
                        assert.strictEqual(args.operations[0].new_attrs.readonly, "1", "Readonly attribute equals to 1");
                        assert.strictEqual(args.operations[0].new_attrs.force_save, "True", '"force_save" sould be equal "True"');
                        changeArch(args.view_id, `
                            <form>
                                <field name="display_name" readonly="1" force_save="True"/>
                            </form>
                        `);
                    } else if (step == 1) {
                        assert.strictEqual(args.operations[1].new_attrs.readonly, "", "Readonly attribute value is empty");
                        assert.strictEqual(args.operations[1].new_attrs.force_save, "False", '"force_save" sould be equal "False"');
                        changeArch(arch);
                    }
                    step++;
                }
        ...
        await click(target.querySelector(".o_web_studio_form_view_editor [name='display_name']"));
    ...
    QUnit.test("fields in arch works correctly", async (assert) => {
        ...
        assert.strictEqual(target.querySelector(".o_web_studio_existing_fields").textContent, "Some FieldIDLast Modified onName")
    })
});
