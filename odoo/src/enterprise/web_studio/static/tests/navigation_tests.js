...
QUnit.module("Studio", (hooks) => {
    ...
    QUnit.test("leaving studio with a pending rendering in Studio", async (assert) => {
        ...
        await createEnterpriseWebClient({ serverData, mockRPC });
        await afterNextRender(() => click(target.querySelector(".o_app[data-menu-xmlid=app_2]")));
        await click(target.querySelector(".o_data_cell"));
        assert.containsOnce(target, ".o_form_view");

        await afterNextRender(() => openStudio(target));
        assert.containsOnce(target, ".o_studio");

        enableRPCWatch = true;
        await click(target.querySelector(".o_web_studio_leave"));
        await afterNextRender(() => {
            // This is low level for simplicity's sake.
            // what matters is that the viewEditor re-renders to the point of making a new FormView
            vem.viewEditorModel.fields.selection.selection.push(["2", "2"]);
        });

        await afterNextRender(() => {
            loadActionDef.resolve();
            return loadActionDef;
        });
        assert.containsNone(target, ".o_studio");
        assert.containsOnce(target, ".o_form_view");

        assert.verifySteps([
            "/web/dataset/call_kw/pony/read",
            "/web/action/load",
            "/web/dataset/call_kw/pony/get_views",
            "/web/dataset/call_kw/pony/read",
        ]);
