...
QUnit.module("Studio Navbar > AppMenuEditor", (hooks) => {
    ...
    QUnit.test("drag/drop to root menu doesn't crash at reorganize menus", async (assert) => {
        assert.expect(2);
        await createAppMenuEditor({
            mockRPC: (route, args) => {
                if (route === "/web/dataset/call_kw/ir.ui.menu/customize") {
        ...
        await click(target, ".o_web_edit_menu");
        // move the submenu at root menu
        await dragAndDrop(
            target.querySelector("li[data-item-id='221'] .o-draggable-handle"),
            target.querySelector(".o-web-studio-interactive-list-remove-item.disabled"),
            "top"
        );
        await click(target, ".o-web-studio-appmenu-editor footer .btn-primary");
    });
