...
class TestFsmTaskAnalysis(TestIndustryFsmCommon):
    def test_tasks_analysis(self):
        ...
        self.env.flush_all()
        task_report = self.env['report.project.task.user'].search_read([('project_id', '=', self.fsm_project.id), ('task_id', '=', self.task.id)], ['remaining_hours', 'progress', 'planned_hours', 'effective_hours', 'working_days_close', 'working_hours_close', 'working_days_open', 'working_hours_open'])[0]
