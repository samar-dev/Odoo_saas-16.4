...
class SaleOrder(models.Model):
    ...
    def _create_mrr_log(self, template_value, initial_values):
        ...
        old_mrr = initial_values.get('recurring_monthly', 0)
        transfer_mrr = 0
        mrr_difference = template_value.get('recurring_monthly') - old_mrr
        ...
        if self.origin_order_id: # Is a confirmed renewal ( origin_order_id and category in progress)
            ...
            if not existing_transfer_log:
                ...
                transfer_mrr = min([template_value.get('recurring_monthly'), parent_mrr])
            ...
            if not float_is_zero(transfer_mrr, precision_rounding=cur_round):
                ...
                mrr_difference = template_value.get('recurring_monthly') - transfer_mrr
        ...
        if not float_is_zero(mrr_difference, precision_rounding=cur_round):
            ...
            mrr_value.update({'event_date': event_date,
                              ...
                              'recurring_monthly': template_value.get('recurring_monthly')})
    ...
    def _create_stage_log(self, values, initial_values):
        ...
        if new_state in SUBSCRIPTION_PROGRESS_STATE + SUBSCRIPTION_CLOSED_STATE and old_state != new_state:
            ...
            if new_state in SUBSCRIPTION_PROGRESS_STATE:
                if confirmed_renewal and self.subscription_id.subscription_state != '6_churn' and not reopened_order:
                    ...
                    parent_mrr_smoothed = max(self.subscription_id.recurring_monthly, 0)
                    parent_mrr = parent_currency._convert(parent_mrr_smoothed,
                    ...
                    event_date = transfer_date
                    if not float_is_zero(values.get('recurring_monthly') - parent_mrr, precision_rounding=cur_round):
                        mrr_change_value = values.copy()
                        mrr_change_value.update({
                            'event_type': self._get_change_event_type(values.get('recurring_monthly') - parent_mrr),
                            'recurring_monthly': values.get('recurring_monthly'),
                            'amount_signed': values.get('recurring_monthly') - parent_mrr,
                            'event_date': transfer_date
                        })
                elif reopened_order:
                    ...
                    if churn_log:
                        ...
                        churn_log.unlink()
                    mrr_difference = values.get('recurring_monthly') - previous_mrr
                    if not float_is_zero(mrr_difference, precision_rounding=cur_round):
                    ...
                    event_type = '0_creation'
                    amount_signed = values.get('recurring_monthly')
                    recurring_monthly = values.get('recurring_monthly')
                else:
                    return
            else:
    ...
    def _mail_track(self, tracked_fields, initial_values):
        ...
        initial_values['recurring_monthly'] = initial_mrr
