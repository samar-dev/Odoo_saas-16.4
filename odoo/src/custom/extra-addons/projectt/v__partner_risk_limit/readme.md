...
- Add the ability to set **risk limit** for customers.
...
- The amount is depended on the company which means, the same customer can have
  multiple `x_risk_limit` depends on the company
- Also, you have the ability to set different `x_risk_limit` as default value
...
2. Activate **Blocage par risque** and set the amount
3. You have the ability to set different amount per partner
4. Go To Sales > Orders > Customers
5. Choose your customer > Sales & Purchases page > **Plafond de risque**
6. You must add yourself to the group **Modifier risque client** in order to modify
   the `x_risk_limit` for the partner
...
| v__partner_credit_limit | needed to access `x_current_outstanding_amount`      |              |
